...
export default function StudentScreen() {
	...
	const handleGoogle = async () => {
		try {
			...
			if (res?.type === 'success' && res.authentication?.accessToken) {
				// TODO: Firebase Google credential ile signIn
	...
	const handleApple = async () => {
		try {
			...
			if (credential?.user) {
				// TODO: Firebase Apple credential ile signIn
	...
	return (
		<KeyboardAvoidingView style={[styles.container, { backgroundColor: colors.surface }]} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
			<ThemedView style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
				<ThemedText type="title" style={[styles.title, { color: colors.textPrimary }]}>Öğrenci Girişi</ThemedText>
				<ThemedText style={[styles.subtitle, { color: colors.textSecondary }]}>Hesabınla giriş yap ve eğitime başla.</ThemedText>
				...
				<View style={styles.registerRow}>
					...
					<TouchableOpacity onPress={() => Alert.alert('Kayıt', 'Kayıt ekranı eklenecek.') }>
						<ThemedText style={{ color: colors.primary, fontWeight: '600' }}>Hemen kayıt ol</ThemedText>
					</TouchableOpacity>
				</View>
			</ThemedView>
