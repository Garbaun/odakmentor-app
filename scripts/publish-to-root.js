...
function publishDocsToRoot(rootDir) {
  ...
  // Flatten duplicated assets path (assets/assets -> assets) for stable URLs
  const nestedAssets = path.join(rootDir, 'assets', 'assets');
  const flatAssets = path.join(rootDir, 'assets');
  if (fs.existsSync(nestedAssets)) {
    console.log('publish-to-root: flattening assets/assets -> assets');
